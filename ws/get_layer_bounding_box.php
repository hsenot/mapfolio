...
$sql = "select st_xmin(b) as xmin,st_ymin(b) as ymin,st_xmax(b) as xmax,st_ymax(b) as ymax from (SELECT ST_Extent(b.the_geom) as b FROM community.building b,community.tag_building tb, community.tag t WHERE b.status <> 9 AND b.id=tb.building_id AND tb.tag_id=t.id AND t.label='".$tag_label."') t";
